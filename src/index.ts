...
// Firebase App Hosting compatibility routes
app.get(
  "/_fah/image/process",
  expressAsyncHandlerWrapper(async (req, res, next) => {
    const operationsString = req.query.operations as string;
...
app.get(
  "/_fah/image/process/**",
  expressAsyncHandlerWrapper(async (req, res, next) => {
    const operationsString = req.url.replace("/_fah/image/process/", "");
    if (!operationsString || !operationsString.length) {
      return next();
    }
    const validatedOperations: ValidatedOperation[] =
...
app.use(function handleError(error: Error, req: Request, res: Response, next: any) {
